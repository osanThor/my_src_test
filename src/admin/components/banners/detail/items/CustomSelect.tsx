...
const CustomSelect = ({
  ...
  const options = [
    { value: 'MAIN', txt: '메인배너' },
    { value: 'SUBSCRIBE_BYBIT', txt: '이용권등록(BYBIT)' },
    { value: 'SUBSCRIBE_BINANCE', txt: '이용권등록(BINANCE)' },
    { value: 'SUBSCRIBE_BITGET', txt: '이용권등록(BITGET)' },
  ];

  useEffect(() => {
    if (place) {
      if (place === 'MAIN') {
        setCurrentValue('메인배너');
      } else if (place === 'SUBSCRIBE_BYBIT') {
        setCurrentValue('이용권등록(BYBIT)');
      } else if (place === 'SUBSCRIBE_BINANCE') {
        setCurrentValue('이용권등록(BINANCE)');
